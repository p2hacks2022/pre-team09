...
void setupConfidence(float degreeOfConfidence, ArrayList<Calendar>  planDateArray, ArrayList<Calendar> deadlineDateArray, ArrayList<Calendar> predictDateArray, ArrayList<Calendar> finishDateArray, boolean isDone[]) {
  for (int i=0; i < planDateArray.size(); i++) {
    ...
    if (isPreDate(finishDateArray.get(i), deadlineDateArray.get(i)) == false) {//finishDate.get(i), deadlineDate.get(i)
...
//引数...Arrayではない
void updateConfidence(Calendar  planDate, Calendar deadlineDate, Calendar predictDate, Calendar finishDate) {
  float alpha = 0;
  float beta = 0;
